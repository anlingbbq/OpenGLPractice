...
public:
	...
	void startup()
	{
		...
		_pbrShader = new Shader("PBR/2.1.2.pbr.vs", "PBR/2.2.1.pbr.fs");
		_equirectangularToCubemapShader = new Shader("PBR/2.1.1.cubemap.vs", "PBR/2.1.1.equirectangular_to_cubemap.fs");
		_irradianceShader = new Shader("PBR/2.1.1.cubemap.vs", "PBR/2.1.2.irradiance_convolution.fs");
		_backgroundShader = new Shader("PBR/2.1.1.background.vs", "PBR/2.1.1.background.fs");
		_prefilterShader = new Shader("PBR/2.1.1.cubemap.vs", "PBR/2.2.1.prefilter.fs");
		_brdfShader = new Shader("PBR/2.2.1.brdf.vs", "PBR/2.2.1.brdf.fs");

		_pbrShader->use();
		_pbrShader->setInt("irradianceMap", 0);
		_pbrShader->setInt("prefilterMap", 1);
		_pbrShader->setInt("brdfLUT", 2);
		_pbrShader->setVec3("albedo", glm::vec3(0.5f, 0.0f, 0.0f));
		...
		float* data = stbi_loadf("../Resources/Texture/hdr/BasketballCourt.hdr", &width, &height, &nrComponents, 0);
		...
		glm::mat4 projection = glm::perspective(glm::radians(_camera->getFOV()), (float)_info.width / (float)_info.height, 0.1f, 100.0f);
		_pbrShader->use();
	...
	void render()
	{
		...
		_pbrShader->setVec3("camPos", _camera->getPosition());
		...
		{
			_pbrShader->setFloat("metallic", (float)row / (float)nrRows);
			for (int col = 0; col < nrColumns; ++col)
			{
				// we clamp the roughness to 0.025 - 1.0 as perfectly smooth surfaces (roughness of 0.0) tend to look a bit off
				// on direct lighting.
				_pbrShader->setFloat("roughness", glm::clamp((float)col / (float)nrColumns, 0.05f, 1.0f));
				...
				_pbrShader->setMat4("model", model);
				renderSphere();
			}
		}
		...
		for (unsigned int i = 0; i < sizeof(lightPositions) / sizeof(lightPositions[0]); ++i)
		{
			...
			_pbrShader->setVec3("lightPositions[" + std::to_string(i) + "]", newPos);
			_pbrShader->setVec3("lightColors[" + std::to_string(i) + "]", lightColors[i]);
		...
		_backgroundShader->use();
		_backgroundShader->setMat4("view", view);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
